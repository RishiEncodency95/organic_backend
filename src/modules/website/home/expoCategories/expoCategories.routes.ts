...
import fs from "fs";
const router = Router();
import * as ctrl from "./expoCategories.controller";
...
router.get('/', (req, res) => ctrl.getExpoCategories(req, res));

// We accept up to 20 images just in case
const uploadFields = Array.from({ length: 20 }).map((_, i) => ({ name: `categoryImage${i}`, maxCount: 1 }));
router.post('/', upload.fields(uploadFields), (req, res) => ctrl.updateExpoCategories(req, res));
