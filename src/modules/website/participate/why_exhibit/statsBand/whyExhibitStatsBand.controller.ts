...
import {
  ...
  updateAllWhyExhibitStatsBandService,
...
export const updateAllWhyExhibitStatsBand = asyncHandler(async (req: Request, res: Response) => {
  const data = await updateAllWhyExhibitStatsBandService(req.body);
  res.status(200).json(new ApiResponse(200, "Stats band items updated successfully", data));
});
